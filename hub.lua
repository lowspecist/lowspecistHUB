-- v1pro — stealth visual hub
-- Sadece client-side güvenli özellikler
...
loadProgress(0.75, "Aimbot modülü hazırlanıyor...")
...
local defaultConfig = {
	...
	Player = { thirdPerson = false, fovChanger = false, noFallCam = false },
...
local function createToggle(parent, text, configTable, key, callback)
	...
	local function upd()
		b.BackgroundColor3 = configTable[key] and theme.btnOn or theme.btnOff
	end
	upd()
...
local function createDropdown(parent, text, options, configTable, key, callback, skipInit)
	...
	task.defer(function()
		if callback and not skipInit then pcall(callback, configTable[key] or options[1]) end
	end)
	return f
end
...
-- ===== Categories =====
local categories = {"Visual", "Combat", "Player", "Server", "Extra"}
					...
					if ch and ch:FindFirstChild("Head") and ch:FindFirstChild("HumanoidRootPart") and ch:FindFirstChild("Humanoid") then
						...
						if cfg.ESP.box then
							if cfg.ESP.boxStyle == "Corner" then
								...
								for _, c in ipairs(corners) do
									...
									if ln then ln.Visible = true; ln.Color = theme.accent; ln.Thickness = 2; ln.From = c[1]; ln.To = c[2]; table.insert(espDrawings, ln) end
									local ln2 = safeDrawing("Line")
									if ln2 then ln2.Visible = true; ln2.Color = theme.accent; ln2.Thickness = 2; ln2.From = c[1]; ln2.To = c[3]; table.insert(espDrawings, ln2) end
								end
							else
								local bx = safeDrawing("Square")
								if bx then bx.Visible = true; bx.Color = theme.accent; bx.Thickness = 1; bx.Filled = false
								bx.Position = Vector2.new(hp.X - boxH/4 + jitter(), hp.Y + jitter()); bx.Size = Vector2.new(boxH/2, boxH)
								table.insert(espDrawings, bx) end
							end
						end
						-- Name
						if cfg.ESP.name then
							local nm = safeDrawing("Text")
							if nm then nm.Visible = true; nm.Color = Color3.fromRGB(255, 255, 255); nm.Center = true; nm.Outline = true; nm.Size = 14
							nm.Position = Vector2.new(hp.X + jitter(), hp.Y - 12); nm.Text = p.Name; table.insert(espDrawings, nm) end
						end
						-- Health
						if cfg.ESP.health then
							...
							if hm.MaxHealth > 0 then
								...
								local hbBg = safeDrawing("Square")
								if hbBg then hbBg.Filled = true; hbBg.Position = Vector2.new(hp.X - boxH/2 - 6, hp.Y); hbBg.Size = Vector2.new(3, boxH); hbBg.Color = Color3.fromRGB(0, 0, 0); table.insert(espDrawings, hbBg) end
								local hb = safeDrawing("Square")
								if hb then hb.Filled = true; hb.Position = Vector2.new(hp.X - boxH/2 - 6, hp.Y + boxH * (1 - hpPct)); hb.Size = Vector2.new(3, boxH * hpPct); hb.Color = Color3.fromRGB(255 * (1 - hpPct), 255 * hpPct, 0); table.insert(espDrawings, hb) end
							end
						end
						-- Distance
						if cfg.ESP.distance then
							local dd = safeDrawing("Text")
							if dd then dd.Visible = true; dd.Color = theme.textDim; dd.Center = true; dd.Size = 12
							local dist = myRoot and math.floor((myRoot.Position - ch.HumanoidRootPart.Position).Magnitude) or 0
							dd.Position = Vector2.new(hp.X, hp.Y + boxH + 4); dd.Text = dist .. "m"; table.insert(espDrawings, dd) end
						end
						-- Tracers
						if cfg.ESP.tracers then
							local tr = safeDrawing("Line")
							if tr then tr.Visible = true; tr.Color = theme.accent; tr.Thickness = 1
							...
							if sm and sm.Z > 0 and st.Z > 0 then tr.From = Vector2.new(sm.X, sm.Y); tr.To = Vector2.new(st.X, st.Y) end
							table.insert(espDrawings, tr) end
						end
						-- Snaplines
						if cfg.ESP.snaplines then
							local sl = safeDrawing("Line")
							if sl then sl.Visible = true; sl.Color = theme.accent; sl.Thickness = 1; sl.Transparency = 0.5
							local bot = Camera:WorldToViewportPoint(ch.HumanoidRootPart.Position)
							sl.From = Vector2.new(Camera.ViewportSize.X / 2, Camera.ViewportSize.Y); sl.To = Vector2.new(bot.X + jitter(), bot.Y + jitter())
							table.insert(espDrawings, sl) end
						end
						-- Head Dot
						if cfg.ESP.headDot then
							local hd = safeDrawing("Circle")
							if hd then hd.Visible = true; hd.Color = Color3.fromRGB(255, 255, 255); hd.Filled = true; hd.Radius = 3
							hd.Position = Vector2.new(hp.X + jitter(), hp.Y + jitter()); table.insert(espDrawings, hd) end
						end
						-- Skeleton
						if cfg.ESP.skeleton then
							local function skLine(a, b)
								...
								if va and vb then
									local ln = safeDrawing("Line")
									if ln then ln.Visible = true; ln.Color = Color3.fromRGB(180, 180, 180); ln.Thickness = 1
									ln.From = Vector2.new(pa.X + jitter(), pa.Y + jitter()); ln.To = Vector2.new(pb.X + jitter(), pb.Y + jitter())
									table.insert(espDrawings, ln) end
								end
							end
...
-- ===== AIMBOT (camera-based, server'a bir şey göndermez) =====
local function startAimbot()
	if aimbotActive then return end
	aimbotActive = true
	if cfg.Aimbot.showFOV then
		fovCircle = safeDrawing("Circle")
		if fovCircle then fovCircle.Visible = true; fovCircle.Color = theme.accent; fovCircle.Thickness = 1; fovCircle.Filled = false
		fovCircle.Radius = cfg.Aimbot.fov; fovCircle.Position = Vector2.new(Camera.ViewportSize.X / 2, Camera.ViewportSize.Y / 2) end
	end
...
UserInputService.InputBegan:Connect(function(input, gp)
	...
	if input.KeyCode == Enum.KeyCode.RightControl then
		if gui.Enabled then disableAllFeatures(); gui.Enabled = false
		else gui.Enabled = true end
	end
...
do
	...
	createToggle(pf, "Third Person", cfg.Player, "thirdPerson", function(on)
		pcall(function()
			local ch = LocalPlayer.Character
			if ch and ch:FindFirstChild("Humanoid") then
				ch.Humanoid.CameraDistanceOffset = on and 12 or 0
			end
		end)
	end)
	createToggle(pf, "FOV Changer", cfg.Player, "fovChanger", function(on)
		Camera.FieldOfView = on and 90 or 70
	end)
	createToggle(pf, "No Fall Camera", cfg.Player, "noFallCam", function(on)
		pcall(function()
			local ch = LocalPlayer.Character
			if ch and ch:FindFirstChild("Humanoid") then
				ch.Humanoid.CameraOffset = on and Vector3.new(0, 2, 0) or Vector3.new(0, 0, 0)
			end
		end)
	end)
