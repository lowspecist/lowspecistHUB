	...
	RunService:BindToRenderStep(espName, 1, function()
		pcall(function()
			...
			for _, p in ipairs(Players:GetPlayers()) do
				if p ~= LocalPlayer and not (cfg.ESP.teamCheck and p.Team == myTeam) then
					local ch = p.Character
					if ch and ch:FindFirstChild("Head") and ch:FindFirstChild("HumanoidRootPart") and ch:FindFirstChild("Humanoid") and ch.Humanoid.Health > 0 then
...
local function startAimbot()
	...
	RunService:BindToRenderStep(aimbotName, 2, function()
		pcall(function()
			...
			for _,p in ipairs(Players:GetPlayers()) do
				if p~=LocalPlayer and not (cfg.Aimbot.teamCheck and p.Team==myTeam) then
					local ch=p.Character
